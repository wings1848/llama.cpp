...
* **可能原因 1**：设置了 `--n-gpu-layers 99`（或设定的 NGL 大于模型实际层数）。此时模型加载器初始会将所有的层放入 GPU。SWLP 的自动探测机制会检测到这一状态，为了避免重复分配和不必要的显存开销，会强制将这些层全部标记为 `layer_fixed_gpu` (静态固定)，跳过滑动迁移逻辑。
  * **解决方案**：若要实现真正的显存节省，请确保您的 NGL 设为 0（`--ngl 0`）或部分显存层（如 `--ngl 16`），同时设定合理的滑动窗口大小（如 `--swlp-window 8`）。
* **可能原因 2**：运行的模型层数原本就小于或等于您设定的窗口大小。例如，对于 16 层的模型，设定了 `--swlp-window 16`。
...
* **解决方案**：
  1. 适当调小滑动窗口，例如显式指定为恒定大小：`--swlp-window 4`，并关闭自适应（`--swlp-adaptive 0`）。
...
* **可能原因 1**：没有开启异步迁移流水线。如果不开启异步，权重拷贝（H2D）会完全阻塞主线程的推理计算。
  * **解决方案**：自 2026-06-11 起，CUDA 后端编译下异步迁移会自动启用。如果您使用此版本之前的代码，需显式加上 `--swlp-async-migration 1`。如果日志显示 `SWLP: async migration disabled`，请确认：
    ...
    2. 运行 `llama-swlp-bench` 时传递 `--swlp-async-migration` 参数（bench 工具不会自动启用）。
...
* **可能原因 3**：窗口设置太小导致频繁发生 GPU 内存重分配。
  * **解决方案**：适当加大窗口（如 `--swlp-window 8`），这会在占用稍微多一点显存的前提下，大幅减少指针失效与显存分配的额外开销。
...
* **原因**：`llama-swlp-bench` 不会像主程序那样自动启用异步迁移，且该工具旧版完全未支持 `--swlp-async-migration` 参数。
* **解决方案**：
  1. 确认使用最新代码（包含 `--swlp-async-migration` 支持的版本）。
  2. 运行测试时加上 `--swlp-async-migration`：
     ```bash
     ./llama-swlp-bench model.gguf --swlp-window 8 --swlp-prefetch 1 --swlp-async-migration
     ```
...
* **推荐方案**：
  * 小模型（<1B）建议关闭 SWLP（`--swlp-window 0`），直接使用纯 CPU 或全 GPU 卸载（`--ngl 99`）。
